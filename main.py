    ...
    async def audio_player_task(self):
        while True:
            ...
            if not self.loop:
                ...
                # reasons.
                try:
                    async with timeout(999999):  # 10 minutes
...
class Commands(commands.Cog):
    ...
    @commands.command(name='jake')
    async def __jake(self, ctx: commands.Context):
        """Zileans true form."""
        await ctx.send(file=discord.File('jakepic.jpg'))

    @commands.command(name='cday')
    async def __cday(self, ctx: commands.Context):
        """Udyr has never looked better."""
        await ctx.send(file=discord.File('colinpic.jpg'))

    @commands.command(name='ivan')
    async def __ivan(self, ctx: commands.Context):
        """Definitley not PC."""
        await ctx.send(file=discord.File('ivanpic.jpg'))

    @commands.command(name='lcs')
    async def __lcs(self, ctx: commands.Context):
        """Current LCS roster."""
        await ctx.send(file=discord.File('lcs.jpg'))
